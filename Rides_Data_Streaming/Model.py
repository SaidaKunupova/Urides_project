...
def dim_location_view():
    df = spark.readStream.table("u_catalog.bronze.silver_obt")
    df = df.select("pickup_city_id", "pickup_city", "city_updated_at",  "state")
...
def fact_view():
    df = spark.readStream.table("u_catalog.bronze.silver_obt")
    df = df.select("ride_id", "pickup_city_id", "driver_id", "payment_method_id", "passenger_id", "vehicle_id","distance_miles", "duration_minutes", "base_fare", "distance_fare", "time_fare", "surge_multiplier", "tip_amount","total_fare","rating", "per_mile", "per_minute")
    df = df.dropDuplicates(subset=['ride_id'])
    return df
...
################################# APPLY FILTER TO EXCLUDE DUPLICATES - CHECK SILVER_OBT NOTEBOOK ##############################
